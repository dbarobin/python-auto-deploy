...
def deploy_api():
    ...
    # Create the build.xml directory.
    run('mkdir -p ~/build-xml-test 2>/dev/null >/dev/null') 
    run('mkdir -p ~/build-xml-test/api 2>/dev/null >/dev/null') 

    # Remove the mall api.
    run('rm -rf '+tomcat_path+'/YOUR_PROJECT*')

    # Copy the mall api build.xml.
    run('cp -v ~/build-xml-test/api/build.xml '+api_path+'')
...
def update_database():
	...
	# Update YOUR_PROJECT Database Settings.
	run('sed -i "s/mall/mall_test/g" '+tomcat_path+'/YOUR_PROJECT/WEB-INF/db.properties')
	# Update YOUR_PROJECT Database Settings.
	run('sed -i "s/mall/mall_test/g" '+tomcat_path+'/YOUR_PROJECT/WEB-INF/db.properties')
