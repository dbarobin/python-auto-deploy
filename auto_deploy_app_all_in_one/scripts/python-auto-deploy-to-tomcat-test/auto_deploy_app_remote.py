...
def main(argv):
    try:
        ...
        if sys.argv[1] == "-h" or sys.argv[1] == "--help":
            ...
            deploy_api()
        elif sys.argv[1] == "-x" or sys.argv[1] == "--update-database":
            update_database()
        else:
...
def usage():
    ...
    usage.append("  [-x | --update-database] Update database settings\n")
...
def update_database():
	...
	print green('Update Database settings.')
	print 'Logs output to the '+log_path+'/update_database.log'

	os.system('mkdir -p '+log_path+' 2>/dev/null >/dev/null')
	os.system("echo '' > "+log_path+"/update_database.log")
	os.system("fab -f "+script_name+" update_database > "+log_path+"/update_database.log")

	print green('Update Database settings finished.')
